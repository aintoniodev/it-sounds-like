...
- [Visualización del soundprint](issues/10-visualizacion-del-soundprint.md): concepto validado — lienzo generativo (manchas por canción, color por energia), palabras del autor en Departure Mono, ficha técnica de dimensiones, historial persistente y export PNG. Issue conocido (lienzo estático entre búsquedas) con fix definido para la construcción.
...
<!-- vacío: todos los tickets resueltos, la niebla restante quedó fuera del destino -->
...
- **Trayectoria temporal del soundprint** (la "línea de plegamiento"): v2 por decisión del DoD; necesita semanas de uso real para existir.
- **Calidad de búsqueda** (tuning, efecto imán) y **enriquecimiento de fichas**: iteración post-v1 con la suite de `eval/`; no son decisiones previas a construir.
